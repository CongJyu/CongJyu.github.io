...
> ⚠️ THE COMMANDS/SCRIPTS IN THIS PASSAGE ARE ALMOST RUN UNDER `root` PRIVILEDGES. PLEASE BE CAREFUL AND MAKE SURE WHAT YOU ARE DOING BEFORE RUNNING THOSE COMMANDS/SCRIPTS IN REAL ENVIRONMENTS.
>
> ⚠️ 本文所包含之命令/腳本幾乎全部都使用 `root` 權限執行。請務必小心並且確保你知道自己喺度做緊乜嘢之後再於真實環境之中執行命令。
...
Copy download link from official [MySQL website](https://downloads.mysql.com/archives/community/) and pull down to machine via `wget` tool. Put the newly downloaded package under the directory `/usr/local`. In this example we need to upgrade MySQL 8.0.36 to MySQL 8.0.42 so we just `wget` the package of MySQL 8.0.42.
...
释放数据库的连接，避免读写不一致的情况发生。

4.2.5. 停止数据库服务

执行 MySQL 脏数据写盘操作，此操作可能需要较长时间，具体耗时取决于脏数据的量和磁盘速度。

> SET GLOBAL innodb_fast_shutdown = 0;

4.2.6. 软链接到新的 MySQL 版本

先移除旧版本的 MySQL 软链接，然后创建新版本的 MySQL 的软链接。
...
备份旧版本的 MySQL 所使用的配置文件 my.cnf，将该文件移动到备份目录之中。

# mv /etc/my.cnf /data/backup/backup_8.0.42.cnf

检查将要用于新版本 MySQL 的配置文件是否符合 MySQL 新版的更新说明，对于已经修改的配置项和已经废弃的配置项，使用新版本的对应替代配置方式进行替代配置。关于由配置变动引起的问题，详见问题排查小节。将新版本要使用的配置放入 /etc 目录之下，存储为 /etc/my.cnf。

4.2.8. 新版本数据库初始化

切换至 /data 目录。将旧版本的 MySQL 相关目录重命名为备份 mysql_bak。

# cd /data
# mv mysql mysql_bak

在 /data 目录下新建新版本 MySQL 所用的相关目录。
...
将 mysql 目录及其子目录的所有者权限更改为 mysql:mysql。

# chown -R mysql:mysql /data/mysql

使用 ls -lah 命令查看详细权限信息，确保修改成功。

# ls -lah | grep mysql
drwxr-xr-x   5 mysql mysql   44 Jul 22 15:33 mysql

确定所有者及用户组为 mysql。初始化 MySQL 数据库。

# /usr/local/mysql/bin/mysql --initialize --user=mysql

4.2.9. 启动数据库

若初始化正常，则可以启动新版本数据库。首先复制一份启动项文件置于 /etc/init.d 目录下。
